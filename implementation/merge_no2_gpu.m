function merge_no2_gpu(start_date, end_date, lat_bounds, lon_bounds, tempo_input_path, tropomi_input_path, data_save_path, options)
    arguments
        start_date string
        end_date string
        lat_bounds double
    ...
    for i = 1:length(run_days)
        ...
        tempo_no2_merge = gpuArray(tempo_no2_merge);
        trop_no2_merge = gpuArray(trop_no2_merge);
        ...
        for chunk_i = 1:num_chunks
            ...
            disp([num2str(100.*chunk_i./num_chunks), ' %'])
        end
        ...
        Pa = (gpuArray.eye(length(Xa)) - K * H) * Pb;
