...
end_day = datetime(2024,6,7, "TimeZone", plot_timezone);
...
data_save_path = '/mnt/disks/data-disk/data/merged_data/temporal';
...
tempo_dim_nc = {"r",tempo_dim(1), "c", tempo_dim(2)};
trop_dim_nc = {"r",trop_dim(1), "c", trop_dim(2)};
...
for i = 1:length(run_days)
    ...
    for j = 1:n_scans
        if ~all(isnan(analysis_no2(:,:,j)), 'all')
            ...
            % For saving to a .nc file
            % savename = ['TEMPO_TROPOMI_merged_', char(datetime(current_day, 'Format', 'uuuuMMdd')), '_S', num2str(scan), suffix, '.mat'];

            % if exist(savename, 'file')
            %     delete(savename) % overwrite files for now
            % end

            % nccreate(savename, 'tempo_no2', 'Dimensions', tempo_dim_nc, 'Format', 'netcdf4');
            % ncwrite(savename, 'tempo_no2', tempo_no2(:,:,j));
            % nccreate(savename, 'tempo_no2_u',  'Dimensions', tempo_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'tempo_no2_u', tempo_no2_u(:,:,j));
            % nccreate(savename, 'tempo_lat',  'Dimensions', tempo_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'tempo_lat', double(tempo_lat(:,:,j)));
            % nccreate(savename, 'tempo_lon',  'Dimensions', tempo_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'tempo_lon', double(tempo_lon(:,:,j)));
            % nccreate(savename, 'tempo_time',  'Dimensions', tempo_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'tempo_time', posixtime(tempo_time(1,:,j)));
            % nccreate(savename, 'tempo_valid_ind',  'Dimensions', tempo_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'tempo_valid_ind', uint8(valid_ind_tempo));

            % nccreate(savename, 'trop_no2',  'Dimensions', trop_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'trop_no2', trop_no2);
            % nccreate(savename, 'trop_no2_u',   'Dimensions', trop_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'trop_no2_u', trop_no2_u);
            % nccreate(savename, 'trop_lat',   'Dimensions', trop_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'trop_lat', double(trop_lat));
            % nccreate(savename, 'trop_lon',   'Dimensions', trop_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'trop_lon', double(trop_lat(:,:,j)));
            % nccreate(savename, 'trop_time',   'Dimensions', trop_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'trop_time', posixtime(trop_time(1,:,:)));
            % nccreate(savename, 'trop_valid_ind',   'Dimensions', trop_dim_nc,'Format', 'netcdf4');
            % ncwrite(savename, 'trop_valid_ind', uint8(valid_ind_trop));

            % nccreate(savename, 'analysis_no2', 'Dimensions', tempo_dim_nc, 'Format', 'netcdf4');
            % ncwrite(savename, 'analysis_no2', analysis_no2(:,:,j));
            % nccreate(savename, 'analysis_no2_u', 'Dimensions', tempo_dim_nc, 'Format', 'netcdf4');
            % ncwrite(savename, 'analysis_no2_u', analysis_no2_u(:,:,j));

            % nccreate(savename, 'tempo_scan', 'Format', 'netcdf4');
            % ncwrite(savename, 'tempo_scan', scan);
