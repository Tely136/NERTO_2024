    ...
    for j = 1:n_scans
        ...
        if ~isempty(tempo_files_scan)
            for k = 1:size(tempo_files_scan,1)
                ...
                tempo_lon_corners(:,:,tempo_step,j) = tempo_data_temp.lon_corners;
                tempo_no2(:,tempo_step,j) = tempo_data_temp.no2 ./ conversion_factor('trop-tempo');
                tempo_no2_u(:,tempo_step,j) = tempo_data_temp.no2_u ./ conversion_factor('trop-tempo');
                tempo_qa(:,tempo_step,j) = tempo_data_temp.qa;
                tempo_cld(:,tempo_step,j) = tempo_data_temp.f_cld;
                tempo_sza(:,tempo_step,j) = tempo_data_temp.sza;
